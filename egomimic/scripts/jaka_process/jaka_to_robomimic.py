...
def add_image_obs(demo_hdf5, demo_obs_group, cam_name):
    ...
    if cam_name == "cam_high":
        ...
        demo_obs_group.create_dataset(
            "wrist_img",
            data=demo_hdf5["observations"]["images"]["cam_wrist"],
            dtype="uint8",
            chunks=(1, 480, 640, 3),
        )
...
def add_xyz_actions(demo_hdf5, demo_i_group, left_extrinsics=None, right_extrinsics=None, prestack=False, POINT_GAP=2, FUTURE_POINTS_COUNT=100):
    """
    demo_hdf5: the demo hdf5 file
    demo_i_group: the demo group to write the data to
    ...
    joint_start = 0
    joint_end = 7
    fk_positions = jaka_fk.fk(demo_hdf5["action"][:, joint_start:joint_end - 1])
    
    extrinsics = left_extrinsics if left_extrinsics is not None else right_extrinsics         
...
def add_ee_pose_obs(demo_hdf5, demo_i_obs_group, left_extrinsics=None, right_extrinsics=None): 
    """
    demo_hdf5: the demo hdf5 file
    ...
    joint_start = 0
    joint_end = 7
    fk_positions = jaka_fk.fk(demo_hdf5["observations"]["qpos"][:, joint_start:joint_end - 1])
...
def process_demo(demo_path, data_group, extrinsics, prestack=False):
    """
    ...
    with h5py.File(demo_path, "r") as demo_hdf5:
        ...
        add_image_obs(demo_hdf5, demo_i_obs_group, "cam_wrist")
        ...
        # add ee_pose
        add_ee_pose_obs(demo_hdf5, demo_i_obs_group, left_extrinsics, right_extrinsics)
        ...
        # actions_xyz
        add_xyz_actions(demo_hdf5, demo_i_group, left_extrinsics, right_extrinsics, prestack=prestack, POINT_GAP=POINT_GAP, FUTURE_POINTS_COUNT=FUTURE_POINTS_COUNT)
...
def  main(args):
    ...
    with h5py.File(args.out, "w", rdcc_nbytes=1024**2 * 2) as dataset:
        ...
        for i, demo in enumerate(tqdm(os.listdir(args.dataset))):
            if not is_valid_path(os.path.join(args.dataset, demo)):
                continue

            demo_path = os.path.join(args.dataset, demo)

            process_demo(demo_path, data_group, EXTRINSICS[args.extrinsics], args.prestack)

    split_train_val_from_hdf5(hdf5_path=args.out, val_ratio=args.train_split, filter_key=None)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        help="path to raw Jaka folder",
    )
    parser.add_argument("--arm", type=str, help="which arm to convert data for")
    parser.add_argument("--train_split", type=float, default=0.2, help="which camera to convert data for")
